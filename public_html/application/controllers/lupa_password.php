...
class Lupa_Password extends Public_Controller
{
    ...
    public function index()
    {
        ...
        $pemesan = (object) $this->input->post(null, true);

        // Data untuk form.
        if (! $_POST) {
            $this->data['values'] = (object) $this->lupa_password->default_values;
        } else {
            $this->data['values'] = $pemesan;
        }
        ...
        $pemesan = (object) $this->input->post(null, true);
        $where = array(
            'id' => get_no_pemesan_value($pemesan->no_pemesan),
            'ktp'  => $pemesan->ktp,
            'email' => $pemesan->email,
        );
        if (! $this->lupa_password->get($where)) {
            $this->session->set_flashdata('pesan_error', 'Informasi yang anda isikan salah, atau Anda tidak terdaftar sebagai pemesan. ' .anchor('lupa-password', 'Ulangi lagi.', 'class="alert-link"'));
            redirect('lupa-password-error');
        }

        // Proses update username dan password
        if (! $this->lupa_password->reset($where)) {
            $this->session->set_flashdata('pesan_error', 'Proses gagal, segera'. anchor('kontak', 'hubungi Operator / Admin Kontrakan.', 'class="alert-link"'));
            redirect('lupa-password-error');
        } else {
